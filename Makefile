...
# Common flags
COMMON_FLAGS = -D'VERSION="${VERSION}"' -Wall -Wextra -pedantic

# C++ specific flags
CXXFLAGS += $(COMMON_FLAGS) $(OPENCV_CFLAGS)
...
install: ${PACKAGE} ${PACKAGE}.1
	install -m755 ${PACKAGE} ${DESTDIR}${PREFIX}/bin/${PACKAGE}
	install -m644 ${PACKAGE}.1 ${DESTDIR}${PREFIX}/share/man/man1/${PACKAGE}.1
